...
import { type CodeLanguage, codeLanguageOptions } from "@/data/languages";
import { cn } from "@/lib/utils";
import { Search, Code2 } from "lucide-react";

interface CodeLanguagePaletteProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

const CodeLanguagePalette = ({
  open,
  onOpenChange,
}: CodeLanguagePaletteProps) => {
  const codeLanguage = useSettingsStore((state) => state.codeLanguage);
  const setCodeLanguage = useSettingsStore((state) => state.setCodeLanguage);
      ...
      switch (e.key) {
        ...
        case "Enter":
          ...
          if (filteredLanguages[selectedIndex]) {
            handleSelect(filteredLanguages[selectedIndex].id as CodeLanguage);
          }
          break;
        case "Escape":
          e.preventDefault();
      ...
      <DialogContent className="p-0 overflow-hidden max-w-md">
          ...
          {filteredLanguages.length === 0 ? (
            ...
            filteredLanguages.map((l, index) => (
              <button
                key={l.id}
                data-index={index}
                onClick={() => handleSelect(l.id as CodeLanguage)}
        ...
        <div className="border-t border-border px-3 py-2 flex items-center justify-between text-xs text-muted-foreground">
          <div className="flex items-center gap-2">
            ...
            <span>{codeLanguageOptions.length} languages</span>
          </div>
...
export default CodeLanguagePalette;
